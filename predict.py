...
xgb_price_model = load_xgboost_model("model/xgboost_model_price_prediction.h5")
xgb_delivery_model = load_xgboost_model("model/xgboost_model_delivery_prediction.h5")
...
if prediction_type == "Prediksi Harga Barang":
    ...
    product_category_encoder = np.load("data/fix/state_encoder.npy",allow_pickle=True).tolist()
    ...
    if st.button("Prediksi Harga"):
        ...
        xgb_price_pred = xgb_price_model.predict(input_data)[0]

        st.write("### Prediksi Harga Barang")
        st.write(f"- **Deep Learning Model**: Rp {dl_price_pred:,.2f}")
        st.write(f"- **XGBoost Model**: Rp {xgb_price_pred:,.2f}")
    ...
    price = st.number_input("Harga Barang (Rp)", min_value=0.0)
    freight_value = st.number_input("Nilai Ongkir (Rp)", min_value=0.0)
    product_weight_g = st.number_input("Berat Produk (gram)", min_value=0.0)
    product_length_cm = st.number_input("Panjang Produk (cm)", min_value=0.0)
    product_height_cm = st.number_input("Tinggi Produk (cm)", min_value=0.0)
    product_width_cm = st.number_input("Lebar Produk (cm)", min_value=0.0)
    ...
    if st.button("Prediksi Lama Pengiriman"):
        ...
        st.write(f"- **Deep Learning Model**: {dl_delivery_pred:.2f} hari")
        st.write(f"- **XGBoost Model**: {xgb_delivery_pred:.2f} hari")
